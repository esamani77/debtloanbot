...
export async function sendOtp(req: Request, res: Response): Promise<void> {
  ...
  if (type === 'phone' && !PHONE_REGEX.test(target)) {
    res.status(400).json({ error: 'Invalid phone number. Use E.164 format (e.g. +989121234567).' });
    return;
  }
...
export async function login(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (msg === 'EMAIL_NOT_VERIFIED' || msg === 'PHONE_NOT_VERIFIED') {
      res.status(403).json({ error: 'Account not verified. Please complete OTP verification.' });
      return;
    }
...
export async function connectTelegramInit(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (msg.includes('BOT_USERNAME')) {
      res.status(500).json({ error: 'Server misconfiguration: BOT_USERNAME not set.' });
      return;
    }
...
export async function connectTelegramStatus(req: Request, res: Response): Promise<void> {
  ...
  if (user.telegramId) {
    res.json({ connected: true, telegramId: user.telegramId, telegramUsername: user.username ?? null });
  } else {
