...
export async function openSplitSession(
  ctx: BotContext,
  sessionId: string,
): Promise<void> {
  const t = useT(ctx.session.userLanguage ?? Language.EN);
  ...
  if (!session) {
    await ctx.reply(t.splitSessionNotFound);
    return;
  }
  ...
  const buttons: ReturnType<typeof Markup.button.callback>[][] = [
    [
      Markup.button.callback(
        t.splitBtnAddBillToSession,
        `split_add_bill:${sessionId}`,
      ),
    ],
  ];
  ...
  if (session.bills.length > 0) {
    buttons.push([
      Markup.button.callback(
        t.splitBtnRecalculate,
        `split_recalculate:${sessionId}`,
      ),
    ]);
  }
  ...
  if (session.status === "CALCULATED" || session.status === "SHARED") {
    if (session.shareToken) {
      buttons.push([
        Markup.button.callback(
          t.splitBtnShareSession,
          `split_reshare:${sessionId}`,
        ),
      ]);
    }
  }
...
export async function addBillToExistingSession(
  ctx: BotContext,
  sessionId: string,
): Promise<void> {
  const t = useT(ctx.session.userLanguage ?? Language.EN);
  ...
  await ctx.reply(t.splitAddingBillToSession(session.name ?? null), {
    parse_mode: "Markdown",
  });
...
export async function recalculateSession(
  ...
  if (session.bills.length === 0) {
    await ctx.reply(t.splitZeroAmountError);
    return;
  }

  try {
    const { netBalances, transfers, shareToken } =
      await calculateSession(sessionId);
    ...
    const balanceSummary = t.splitBalanceSummary(
      session.participants,
      netBalances,
      sym,
    );
    ...
    await ctx.editMessageText(`${balanceSummary}\n\n${plan}\n\n${shareMsg}`, {
      parse_mode: "Markdown",
    });
    ...
    const invites = await prisma.splitParticipantInvite.findMany({
      where: { splitSessionId: sessionId },
    });
    if (invites.length > 0) {
      const links = invites.map((inv) => ({
        name:
          session.participants[inv.participantIndex] ??
...
export async function reshareSession(
  ctx: BotContext,
  sessionId: string,
): Promise<void> {
  const t = useT(ctx.session.userLanguage ?? Language.EN);
